...
export function barPanel(a: VisionAnalysis): string {
  ...
  }<dl class="bar-metrics"><div><dt>Horizontal deviation</dt><dd>${metrics ? metrics.horizontal.toFixed(1) : '—'}<small>% frame width</small></dd></div><div><dt>Vertical rise</dt><dd>${metrics ? metrics.rise.toFixed(1) : '—'}<small>% frame height</small></dd></div><div><dt>Peak upward velocity</dt><dd>${metrics?.velocity !== null && metrics?.velocity !== undefined ? metrics.velocity.toFixed(1) : '—'}<small>% frame height / s</small></dd></div></dl></div><p class="footnote">Compares your estimated wrist path (green) against the ideal trajectory and error corridor (blue dashed envelope). Dashed vertical line: center vertical axis.</p></section>`;
}
