import { relativeBarMetrics } from '../domain/automatic-bar';
...
export function techniqueFeedback(a: VisionAnalysis, canSeek: boolean): string {
  ...
  return `<section aria-label="Technique observations"><div class="section-title"><h2>The details that matter</h2><span class="micro">${checks.length} MEASURED CHECKS</span></div>${checks.length ? checks.map((c, i) => `<details class="issue measured-issue" ${!c.passed ? 'open' : ''}><summary class="issue-toggle"><span class="issue-number">${String(i + 1).padStart(2, '0')}</span><span class="issue-name"><strong>${e(c.name)}</strong><small>${c.value}${e(c.unit)} measured · target ≥${c.target}${e(c.unit)}</small><span class="severity ${c.passed ? 'check-met' : 'moderate'}">${c.passed ? 'Check met' : 'Review'}</span></span><span class="expand-icon" aria-hidden="true">+</span></summary><div class="issue-body"><button class="text-link" data-cv-time="${c.time}" ${canSeek ? '' : 'disabled'}>VIEW FRAME · ${c.time.toFixed(2)} s ${icon('arrow')}</button><dl><dt>What was measured</dt><dd>${e(c.detail)}</dd><dt>Result</dt><dd>${c.value}${e(c.unit)} ${c.passed ? 'meets' : 'is below'} this prototype’s ${c.target}${e(c.unit)} threshold${c.passed ? '' : ` by ${c.target - c.value}${e(c.unit)}`}.</dd><dt>What to review</dt><dd>${c.passed ? 'Inspect the surrounding phase and calibrated bar path; this angle alone does not establish a successful lift.' : 'Replay this moment and check the joint overlay against the actual limb position. If the measurement is accurate, discuss this position with a coach before changing technique.'}</dd></dl></div></details>`).join('') : '<p class="notice">No technique score assigned. No recognized phase has enough reliable evidence for a measurement check. Try a steady side view with the whole athlete visible.</p>'}<details><summary>How the score is calculated</summary><p class="footnote">Score = checks met ÷ available checks × 100. Missing checks are excluded, not failed. Up to five visible-angle checks are assessed: pull elbow ≥160°, end-of-pull hip and knee ≥165°, receiving elbow ≥165°, standing recovery knee ≥165°. These are transparent prototype rules, not validated coaching standards. Tracking coverage is separate. A 100 does not establish a good or successful lift; balance, bar path, timing and individual anatomy are not included.</p></details></section>`;
}
export function barPanel(a: VisionAnalysis, canTrack: boolean): string {
  if (!a.bar && a.automaticBar) {
    const t = a.automaticBar,
      m = relativeBarMetrics(t),
      first = t.points[0];
    const path = t.points
      .map((p, i) => `${i ? 'L' : 'M'}${p.x},${p.y}`)
      .join(' ');
    return `<section class="bar-section" aria-label="Measured bar path"><div class="section-title"><h2>Follow the bar</h2><span class="micro">AUTOMATIC · VERIFY PLATE</span></div><div class="bar-content measured-bar-content"><svg class="measured-bar-path" viewBox="0 0 ${t.width} ${t.height}" role="img" aria-label="Automatic candidate plate path with vertical reference"><path d="M${first.x},0V${t.height}" stroke="#a8b09d" stroke-dasharray="4 4"/><path d="${path}" fill="none" stroke="#d4f778" stroke-width="2"/></svg><dl class="bar-metrics"><div><dt>Horizontal deviation</dt><dd>${m.horizontal.toFixed(1)}<small>% frame width</small></dd></div><div><dt>Vertical rise</dt><dd>${m.rise.toFixed(1)}<small>% frame height</small></dd></div><div><dt>Physical velocity</dt><dd>—<small>size calibration needed</small></dd></div></dl></div><p class="notice">Experimental automatic plate candidate. Verify the blue marker on your video before interpreting this path. ${t.stoppedEarly ? 'Tracking lost confidence; only the initial part is shown.' : ''}</p><p class="footnote">Dashed line: vertical through the first measured point, not a prescribed ideal trajectory. Camera movement and perspective affect this comparison. No physical plate diameter is assumed.</p>${canTrack ? '<button class="secondary" data-action="track-bar">Calibrate and track bar</button>' : ''}</section>`;
  }
  const track = a.bar,
