...
export function barPanel(a: VisionAnalysis): string {
  ...
  const model = getBarPathModel(a.exercise?.id ?? 'snatch');
  ...
  if (track && track.points.length >= 2) {
    ...
    const yTargetSpan = model.startY - model.topY;
    const scale = yTargetSpan / ySpan;

    track.points.forEach((p, i) => {
      const normY = (maxY - p.y) / ySpan;
      const svgY = model.startY - normY * yTargetSpan;
  ...
  return `<section class="bar-section analysis-card" aria-label="Estimated bar path"><div class="section-title"><h2>Follow the bar</h2><span class="micro">${e(model.exerciseName.toUpperCase())} · WRIST-LINE ESTIMATE</span></div><div class="bar-content measured-bar-content">${
    hasPath
      ? `<svg class="measured-bar-path" viewBox="115 15 150 355" role="img" aria-label="Estimated wrist trajectory for ${e(model.exerciseName)} compared against expected trace error margin and athlete silhouette"><path d="${model.silhouette.legs}" fill="none" stroke="#2d3824" stroke-width="12" stroke-linecap="round" stroke-linejoin="round"/><path d="${model.silhouette.arms}" fill="none" stroke="#2d3824" stroke-width="11" stroke-linecap="round" stroke-linejoin="round"/><circle cx="${model.silhouette.head[0]}" cy="${model.silhouette.head[1]}" r="13" fill="#2d3824"/><path d="M194 30V354" stroke="#48573b" stroke-dasharray="3 5" stroke-width="1.2"/><path d="${model.corridorPath}" fill="#73c9ff" fill-opacity="0.12" stroke="#73c9ff" stroke-opacity="0.3" stroke-width="1" stroke-dasharray="2 3"/><path d="${model.refTracePath}" fill="none" stroke="#73c9ff" stroke-width="2.2" stroke-dasharray="4 4" stroke-linecap="round"/><circle cx="${model.refPoints[model.refPoints.length - 1][0]}" cy="${model.refPoints[model.refPoints.length - 1][1]}" r="3.5" fill="#73c9ff"/><path d="${measuredSvgPath}" fill="none" stroke="#d4f778" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/>${startDot}${endDot}</svg><div class="bar-path-legend"><span class="legend-badge"><span class="legend-indicator legend-corridor-indicator"></span> Expected trace (± margin)</span><span class="legend-badge"><span class="legend-indicator legend-user-indicator"></span> Your wrist path</span></div>`
