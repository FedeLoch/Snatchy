...
export function shell(content: string, page: Page, theme: Theme): string {
  ...
  return `<a href="#main" class="skip-link">Skip to content</a><header class="app-header"><a class="brand" href="#home" aria-label="Snatchy home"><img class="brand-logo" src="/logo-snatchy.png" alt="" aria-hidden="true"><span>snatchy<span>.</span></span></a><div class="edition">THE TECHNIQUE LAB <span>VOL. 01</span></div><div class="header-tools"><span class="header-status"><i></i> LOCAL FIRST</span>${toggle}</div></header><main id="main" tabindex="-1">${content}</main><nav aria-label="Main navigation">${(['home', 'capture', 'history'] as const).map((p) => `<a href="#${p}" ${page === p || (page === 'result' && p === 'capture') ? 'aria-current="page"' : ''}>${icon(p === 'capture' ? 'plus' : p)}<span>${p === 'capture' ? 'Analyze' : p === 'home' ? 'Home' : 'History'}</span></a>`).join('')}</nav><footer class="app-footer"><span class="footer-credit">Built by <a class="creator-link" href="https://github.com/FedeLoch" target="_blank" rel="noopener noreferrer"><strong>Fede Lochbaum</strong></a> · <a class="donate-link" href="https://buymeacoffee.com/fedelochbaum" target="_blank" rel="noopener noreferrer">☕ Buy me a coffee</a></span></footer><div class="sr-only" id="announcer" role="status" aria-live="polite"></div>`;
}
