...
function rows(records: LiftRecord[]): string {
  return records.length
    ? `<div class="lift-list">${records.map((r) => `<a class="lift-row" href="#result/${r.id}" aria-label="Open ${e(getMovement(r.analysis.movementId)?.name)} ${r.source === 'video' ? 'upload, not analyzed' : 'result, score ' + r.analysis.score}"><span class="lift-icon">${icon('arrow')}</span><span class="lift-description"><strong>${e(getMovement(r.analysis.movementId)?.name)}</strong><small>${e(new Date(r.createdAt).toLocaleString(undefined, { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }))}<span class="row-dot">·</span>${r.source === 'video' ? 'Imported clip' : 'Demo lift'}</small></span>${r.source === 'video' ? '<span class="upload-status">Not analyzed</span>' : `<span class="lift-score">${r.analysis.score}<small>/100</small></span>`}${icon('arrow')}</a>`).join('')}</div>`
...
export function home(records: LiftRecord[], warning: string): string {
  return `<div class="home-grid"><section class="home-intro"><div class="eyebrow"><i></i> BUILT FOR THE MOMENTS BETWEEN SETS</div><h1>Every lift.<br>A little <em>better.</em></h1><p class="lead">See the details. Understand the movement.<br>Make your next rep count.</p><a class="primary" href="#capture">Analyze a lift ${icon('arrow')}</a><span class="under-cta">ONE LIFT. A CLEARER PERSPECTIVE.</span></section><button class="demo-hero" data-action="demo" aria-label="Explore the Snatch demo"><span class="hero-head"><span><strong>THE SNATCH</strong><small>POWER. PRECISION. PERSPECTIVE.</small></span><span class="tag">INTERACTIVE DEMO</span></span>${movementVisual('snatch', 1.34, true, 'elbow')}<span class="hero-foot"><span><i></i> SEE WHAT YOU’VE BEEN MISSING</span>${icon('arrow')}</span></button></div><section class="recent-section"><div class="section-title"><h2>Recent lifts</h2><a href="#history" class="text-link">View history ${icon('arrow')}</a></div>${warning ? `<p class="notice" role="status">${e(warning)}</p>` : ''}${rows(records.slice(0, 3))}</section><div class="principle-strip"><span>01 <b>Record</b></span><span>02 <b>Understand</b></span><span>03 <b>Improve</b></span></div>`;
}
export function history(records: LiftRecord[], warning: string): string {
  return `<div class="page-title"><div class="eyebrow">A RECORD OF YOUR WORK</div><h1>Lift history<span class="accent">.</span></h1><p>Demo results are saved on this device. Uploaded videos have not been analyzed.</p></div>${warning ? `<p class="notice" role="status">${e(warning)}</p>` : ''}${rows(records)}<p class="footnote">Video files are not saved. Earlier uploads are marked as not analyzed; their old demo scores are not valid technique scores.</p><a class="primary compact" href="#capture">Analyze a lift ${icon('plus')}</a>`;
}
